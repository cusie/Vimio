...
public class JwtLoginFilter extends AbstractAuthenticationProcessingFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ResponseResult result = ResponseResult.create(400, "非法请求");
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response,
		...
		map.put("user", user);
		map.put("token", jwt);
		ResponseResult result = ResponseResult.ok("登录成功", map);
		PrintWriter out = response.getWriter();
		out.write(JacksonUtils.writeValueAsString(result));
		out.flush();
		out.close();
/*		LoginLog log = handleLog(request, true, "登录成功");
	...
	protected void unsuccessfulAuthentication(HttpServletRequest request, HttpServletResponse response,
		...
		PrintWriter out = response.getWriter();
		out.write(JacksonUtils.writeValueAsString(ResponseResult.create(401, msg)));
		out.flush();
		out.close();
