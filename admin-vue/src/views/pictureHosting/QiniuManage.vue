<template>
  <div>
    <el-row>
      <el-select v-model="qiniuConfig.bucketName" disabled style="min-width: 200px"></el-select>
    ...
    <el-alert title="相关提示信息" type="warning" show-icon close-text="不再提示" v-if="hintShow1" @close="noDisplay(1)"></el-alert>
    <el-alert title="更多提示信息" type="warning" show-icon close-text="不再提示" v-if="hintShow2" @close="noDisplay(2)"></el-alert>
    <el-row v-viewer>
      <div class="image-container" v-for="(file,index) in fileList" :key="index">
        ...
        <div class="image-content">
          ...
          <div class="icons">
            <el-tooltip class="item" effect="dark" content="复制 CDN 链接" placement="bottom">
              <i class="icon el-icon-link" @click="copy(1,file)"></i>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" content="复制 MD 格式" placement="bottom">
    ...
    <el-drawer title="上传文件" :visible.sync="isDrawerShow" direction="rtl" size="40%" :wrapperClosable="false" :close-on-press-escape="false">
      <el-row>
        ...
        <el-radio v-model="nameType" label="2">使用 UUID 文件名</el-radio>
...
import { getBucketContents, delFile, upload } from "@/api/qiniu";
...
export default {
  ...
  created() {
    this.hintShow1 = localStorage.getItem('qiniuHintShow1')? false : true;
    this.hintShow2 = localStorage.getItem('qiniuHintShow2')? false : true;

    const qiniuConfig = localStorage.getItem('qiniuConfig');
    if (qiniuConfig) {
      this.qiniuConfig = JSON.parse(qiniuConfig);
    } else {
      this.msgError('请先配置七牛云');
  ...
  methods: {
    ...
    async getReposContents(arr, path) {
      await getBucketContents(this.qiniuConfig.bucketName, path).then(res => {
    ...
    search() {
      ...
      getBucketContents(this.qiniuConfig.bucketName, path).then(res => {
        res.files.forEach(item => {
    ...
    noDisplay(id) {
      localStorage.setItem(`qiniuHintShow${id}`, '1');
    },
    imgPath(file) {
      let path = this.activePath.join('/');
    ...
    imgCdnUrl(file) {
      return `${this.qiniuConfig.domain}${this.imgPath(file)}`;
    },
    copy(type, file) {
    ...
    delFile(file) {
      this.$confirm("此操作将永久删除该文件, 是否删除?", "提示", {
        ...
        delFile(this.qiniuConfig.bucketName, file.path).then(() => {
    ...
    upload(data) {
      ...
      upload(this.qiniuConfig.bucketName, this.realPath, fileName, data.file).then(() => {
