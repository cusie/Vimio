...
<!--    <el-card>
      ...
      <el-form :model="qiniuConfig" label-width="100px">
        ...
        <el-form-item label="CDN访问域名">
          <el-input v-model="qiniuConfig.domain"></el-input>
        </el-form-item>
        <el-button type="primary" size="medium" icon="el-icon-check" :disabled="!isQiniuSave" @click="saveQiniu(true)">保存配置</el-button>
        <el-button type="info" size="medium" icon="el-icon-close" @click="saveQiniu(false)">清除配置</el-button>
      </el-form>
    </el-card>-->
...
export default {
	...
	data() {
		return {
			...
			hintShow: false,
      qiniuConfig: {
        accessKey: 'Zk0dQF--rKgVvqzGJ6h6tskZe7OZbmevW-aXJlAj',
        secretKey: 'XQEMCD9F4XEcxsRJIZs1J5fyx1Q93ayuBnwcgZtu',
				bucketName: 'vimio',
				domain: 'https://api.vimio.top'
			},
			upyunConfig: {
				username: '',
				password: '',
	...
	computed: {
    isQiniuSave() {
      return this.qiniuConfig.accessKey && this.qiniuConfig.secretKey && this.qiniuConfig.bucketName && this.qiniuConfig.domain;
    },
		isUpyunSave() {
    ...
    saveQiniu(save) {
      if (save) {
        localStorage.setItem('qiniuConfig', JSON.stringify(this.qiniuConfig));
        this.msgSuccess('保存成功');
      } else {
        localStorage.removeItem('qiniuConfig');
        this.msgSuccess('清除成功');
      }
    }
    ,
